...
const BoxInformations = (props: IProps) => {
    ...
    return (
        <div className="box-informations">
            <span className={props.activity}><i className={props.icon}></i> { props.format ? formatTitle(props.quantity, props.message) : props.message}</span>
            <span>-</span>
            <span className={`quantity-value ${props.activity}`}>{props.quantity}</span>
        </div>
    )
}
