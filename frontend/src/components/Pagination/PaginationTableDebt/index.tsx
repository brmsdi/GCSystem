...
import { paginationDebtTableAction, updateDebtsTable } from "store/Debts/debts.actions";
import { selectCurrentPaginationTableDebts, selectUpdateTableDebtCurrentState } from "store/Debts/debts.selector";
...
const PaginationTableDebt = () => {
  ...
  const currentPaginationTable: PaginationTableAction = useSelector(selectCurrentPaginationTableDebts)
  const [propsPagination, setPropsPagination] = useState<PropsPagination>()
  const updateTableDebtCurrentState = useSelector(selectUpdateTableDebtCurrentState);
  useEffect(() => {
    try {
        ...
        findByCPFService(currentPaginationTable.search)
          .then(async (response) => {
            ...
            setPropsPagination(paginationInformations)
             dispatch(updateDebtsTable(currentPaginationTable.currentPage, response.data))
          })
      } else {
        getAllDebts(currentPaginationTable.currentPage)
          .then(async (response) =>  {
            const paginationInformations = await createNumberPages(currentPaginationTable.currentPage, response.data.totalPages)
            setPropsPagination(paginationInformations)
            dispatch(updateDebtsTable(currentPaginationTable.currentPage, response.data))
