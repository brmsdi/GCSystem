...
import { paginationLesseeTableAction, updateLesseesTable } from "store/Lessees/lessees.actions";
import { selectCurrentPaginationTableLessees, selectUpdateTableLesseeCurrentState } from "store/Lessees/lessees.selector";
...
const PaginationTableLessee = () => {
  ...
  const currentPaginationTable: PaginationTableAction = useSelector(selectCurrentPaginationTableLessees)
  const [propsPagination, setPropsPagination] = useState<PropsPagination>()
  const updateTableLesseeCurrentState = useSelector(selectUpdateTableLesseeCurrentState);
  useEffect(() => {
    try {
        ...
        findByCPFService(currentPaginationTable.search)
          .then(async (response) => {
            ...
            setPropsPagination(paginationInformations)
             dispatch(updateLesseesTable(currentPaginationTable.currentPage, response.data))
          })
      } else {
        getAllLessees(currentPaginationTable.currentPage)
          .then(async (response) =>  {
            const paginationInformations = await createNumberPages(currentPaginationTable.currentPage, response.data.totalPages)
            setPropsPagination(paginationInformations)
            dispatch(updateLesseesTable(currentPaginationTable.currentPage, response.data))
