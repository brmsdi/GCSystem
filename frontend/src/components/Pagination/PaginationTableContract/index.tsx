...
import {
  ...
  selectUpdateTableContractCurrentState,
} from "store/Contracts/contracts.selector";
...
const PaginationTableContract = () => {
  ...
  const updateTableContractCurrentState = useSelector(
    selectUpdateTableContractCurrentState
  );
  useEffect(() => {
    try {
      if (currentPaginationTable.search) {
        ...
        getAllContracts(currentPaginationTable.currentPage).then(
          async (response) => {
            const paginationInformations = await createNumberPages(
              currentPaginationTable.currentPage,
              response.data.totalPages
