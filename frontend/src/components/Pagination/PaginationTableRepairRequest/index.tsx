...
import { findByCPFService, getAllRepairRequests } from "services/repair-request";
import { paginationRepairRequestTableAction, updateRepairRequestTable } from "store/RepairRequests/repair-requests.actions";
import { selectCurrentPaginationTableRepairRequests, selectUpdateTableRepairRequestCurrentState } from "store/RepairRequests/repair-requests.selector";
...
const PaginationTableRepairRequest = () => {
  ...
  const currentPaginationTable: PaginationTableAction = useSelector(selectCurrentPaginationTableRepairRequests)
  const [propsPagination, setPropsPagination] = useState<PropsPagination>()
  const updateTableRepairRequestCurrentState = useSelector(selectUpdateTableRepairRequestCurrentState);
  useEffect(() => {
    try {
        ...
        findByCPFService(currentPaginationTable.search)
          .then(async (response) => {
            ...
            setPropsPagination(paginationInformations)
             dispatch(updateRepairRequestTable(currentPaginationTable.currentPage, response.data))
          })
      } else {
        getAllRepairRequests(currentPaginationTable.currentPage)
          .then(async (response) =>  {
            const paginationInformations = await createNumberPages(currentPaginationTable.currentPage, response.data.totalPages)
            setPropsPagination(paginationInformations)
            dispatch(updateRepairRequestTable(currentPaginationTable.currentPage, response.data))
