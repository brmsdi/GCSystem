...
import { selectSelectedRepairRequestsOrderService, selectStateModalOrderServiceRepairRequests } from "store/RepairRequests/repair-requests.selector";
...
const FormTemplate = (props: IProps) => {
  ...
  const selectedRepairRequests : RepairRequest[] = useSelector(selectSelectedRepairRequestsOrderService)
  ...
  useEffect(() => {
    checkLegend('fieldset-order-service', 
  ...
  async function submit(event: any) {
    ...
    if (isEmpty(selectedRepairRequests)) {
      Swal.fire('Oops!', 'Adicione solicitações de reparo a essa ordem de serviço', 'error')
      return
    }

    if (isEmpty(selectedEmployees)) {
      Swal.fire('Oops!', 'Adicione funcionários a essa ordem de serviço. Eles serão responsáveis por executar os reparos.', 'error')
      return
    }
  ...
  function changeModalSelectRepairRequests() {
    dispatch(changeStateModalOrderServiceRepairRequests({ isOpen: !stateModalOrderServiceRepairRequests.isOpen}))
  }
        ...
        <div className="row-form-1">
          ...
          <div className="form-container f4">
            ...
            <input
              ...
              name="reservedDate"
              value={form.reservedDate.length > 0 ? formatDate(form.reservedDate) : form.reservedDate}
          ...
          <div className="form-container f4">
            ...
            <input
              ...
              name="completionDate"
              value={form.completionDate.length > 0 ? formatDate(form.completionDate) : form.completionDate}
              onChange={(e) => changeInput({ completionDate: e.target.value })}
            />
          </div>
          <div className="form-container f4">
